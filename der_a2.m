...
p = struct(...                   % Reference: 2023 - NERC - Parameterization of the DER_A Model for Aggregate DER.pdf
    ...
    'Idmax'    ,  2.9      , ... % maximum limit of total active current (or Ipmax)
    'Idmin'    , -2.9      , ... % minimum limit of total active current (or Ipmin)
    'Imax'     ,  1.2      , ... % maximum converter current (pu)
    'Iqh1'     ,  1.2        , ... % maximum limit of reactive current injection, p.u.
    'Iql1'     , -1.2        , ... % minimum limit of reactive current injection, p.u.    
...
function dx = dera_indexed(~, x, p, Vmag, P, Q, f, k)
    ...
    dx(4) = -1/p.Tg * x(4) + 1/p.Tg * aux_fcn_SSF(x(3) - aux_fcn_SSF(aux_fcn_Iqv(p, x), p.Iqh1, p.Iql1, p.k), p.Iqmax, p.Iqmin, p.k);       % s3
     ...
     dx(5) = -1/p.Tv  * x(5) + 1/p.Tv * TripVoltageLogic2(k, x(1), p.vl0, p.vl1, p.vh0, p.vh1, p.Vrfrac, p.tvl1, p.tvl0, p.tvh1, p.tvh0);

    dx(6) = -1/p.Trf * x(6) + freq_now/ p.Trf;                                                           % s5
    dx(7) = p.kig * aux_fcn_A_2(p, x, Pref_now) + p.kw * (aux_fcn_B(p, x) - p.kpg * aux_fcn_A_2(p, x, Pref_now) - x(7));
                                                                                                                          % s6
    dx(8) = aux_fcn_SSF( ...
                -1/p.Ts * x(8) + 1/p.Ts * aux_fcn_B(p, x), ...
                p.dpmax, p.dpmin, p.k);                      
