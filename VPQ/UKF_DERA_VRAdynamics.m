...
% === x1: Filtered terminal voltage ===
s0_0 = Vpu;  % Initial filtered voltage (x1)
...
s2_0 = aux_fcn_SSF(s1_0 - Iq_ref_0, p.Iqmax, p.Iqmin, p.k);  % Initial x4

% === x4: Active power control (s3) ===
s3_0 = P_total(1);  
% === x5: D-axis current control (s4) ===
x_Idpre = [s0_0; s3_0];  % Only voltage (x1) and power control (x4) are needed
...
%% Voltage-Reactive Dynamics: Initialization (5 States + 5 Parameters)
n_states = 5;
n_param  = 5;
...
freq_now = f_802 / 60;
%% 
% RK4 propagation loop for full VRADYNAMICS: 5 states + 7 parameters
...
for k = 2:Nt-1
    ...
    % Compute RK4 stages using updated vradynamics functions
    ...
    % Final RK4 update for all 12 states (dynamic states + parameters)
    x_true(:, k) = x_true(:, k-1) + (1/6) * (k1 + 2*k2 + 2*k3 + k4);
end

% Time vector

tt = t(1:Nt-1);  % Time vector for plotting
figure;
...
for i = 1:10
    subplot(4, 3, i);  % Grid size remains 4x3 for spacing
...
sgtitle('RK4 Trajectories of VRADYNAMICS States and Parameters');


%% %% === UKF Setup and Initialization ===
n = 10;  % 5 states + 5 parameters
...
Q = (1e-4)^2 * eye(n);    % Process noise
R = (1e-2)^2 * eye(3);    % Measurement noise for [Vt, Q, P]

% Initial State Estimate
xhat_UKF(:,1) = [s0_0; s1_0; s2_0; s3_0; s4_0; ...
                 0.01; p.kqv*0.8; p.Tg; p.Tiq+0.05; p.Tpord+0.02];

% Initial Covariance
P_UKF(:,:,1) = diag([0.01 * ones(1,5), 0.1 * ones(1,5)]);
...
for k = 2:Nt-1
    ...
    A = gamma * chol(Pk* eye(n), 'lower');
    X_sigma = [xk, xk + A, xk - A];

    % --- RK4 Propagation of Sigma Points ---
    X_sigma_pred = zeros(n, 2*n+1);
    for i = 1:(2*n+1)
    xi = X_sigma(:,i);  %  Current sigma point

    k1 = compute_k1_vradynamics(xi, k, dt, p, Vt_now, Qref_now, freq_now, Pref_now);
    k2 = compute_k2_vradynamics(xi, k1, k, dt, p, Vt_now, Qref_now, freq_now, Pref_now);
    k3 = compute_k3_vradynamics(xi, k2, k, dt, p, Vt_now, Qref_now, freq_now, Pref_now);
    k4 = compute_k4_vradynamics(xi, k3, k, dt, p, Vt_now, Qref_now, freq_now, Pref_now);
    ...
    Y_sigma = zeros(3, 2*n+1);
    for i = 1:(2*n+1)
        x1 = X_sigma_pred(1,i);
        x3 = X_sigma_pred(3,i);
        x5 = X_sigma_pred(5,i);
        Y_sigma(:,i) = [x1; x1*x3; x1*x5];  % Measurement model: [Vt; Q; P]
    end
    y_pred = Y_sigma * Wm';
    ...
    K = Pxy / Pyy;
    y_k = [x_true(1,k); x_true(1,k)*x_true(3,k); x_true(1,k)*x_true(5,k)];
    xhat_UKF(:,k) = x_pred + K * (y_k - y_pred);

% === Clamping dynamic states ===
xhat_UKF(1:5,k) = min(max(xhat_UKF(1:5,k), -1e6), 1e6);   % states like Vt, Iq, Id, etc.
xhat_UKF(6:10,k) = max(xhat_UKF(6:10,k), 1e-4);           % parameters must remain positive
...
for i = 1:5
    for k = 1:Nt
        ukf_var(i,k) = P_UKF(i+5,i+5,k);
    end
end
save('UKF_variance.mat', 'ukf_var');
...
for i = 1:5
    subplot(3,2,i);
    plot(t_long(1:Nt-1), xhat_UKF(i+5,1:Nt-1), 'b-', 'LineWidth', 1.5); hold on;
    plot(t_long(1:Nt-1), true_params(i,1:Nt-1), 'r--', 'LineWidth', 1.5);
    title(['UKF Estimate of ', param_labels{i}]);
    xlabel('Time (s)'); ylabel(param_labels{i});
    legend('Estimated', 'True'); grid on;
end
sgtitle('UKF Parameter Estimation');

% %% === Plotting: State Estimates ===
% state_labels = {'Vt', 'iqpfc', 'Iq', 'Pgen', 'Id'};
% 
% figure('Name','UKF State Estimates','NumberTitle','off');
% for i = 1:5
%     subplot(3,2,i);
%     plot(t_long(1:Nt-1), xhat_UKF(i,1:Nt-1), 'b-', 'LineWidth', 1.5); hold on;
%     plot(t_long(1:Nt-1), x_true(i,1:Nt-1), 'r--', 'LineWidth', 1.5);
%     title(['UKF Estimate vs True: ', state_labels{i}]);
%     xlabel('Time (s)'); ylabel(state_labels{i});
%     legend('Estimated', 'True'); grid on;
% end
% sgtitle('UKF Dynamic State Estimation');

%% % === SAVE UKF RESULTS ===
UKF_estimates = xhat_UKF;   % UKF output (10 x Nt)

save('UKF_results.mat', 'UKF_estimates');
disp('✔ UKF results saved to UKF_results.mat');
