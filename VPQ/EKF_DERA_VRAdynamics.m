...
%% EKF and Simulation: DER_A Voltage-Reactive Dynamics (5 States + 5 Parameters)
n_states = 5;
n_param  = 5;
n_total  = n_states + n_param;
dt  = 0.02;
...
R = (1e-4)^2 * eye(3);                 % Measurement noise for [Vt, Q, P]
I = eye(n_total);
...
%% EKF Loop
%% === EKF Estimation Loop: Voltage-Reactive Dynamics ===
for k = 2:Nt-1
    % --- Prior State ---
    xk = xhat_plus(:, k-1);
    ...
    % % --- Clamp Predicted State ---
    % xhat_minus(1:5,k)   = min(max(xhat_minus(1:5,k), -1e3), 1e3);
    xhat_minus(6:10,k)  = max(xhat_minus(6:10,k), 1e-6);  % enforce positivity
    ...
    P_minus(:,:,k) = P_minus(:,:,k) + 1e-6 * eye(n_total);  % regularization
    ...
    Hk = zeros(3, n_total);
    Hk(1,1) = 1;             % ∂Vt/∂x1
    Hk(2,1) = x3;  Hk(2,3) = x1;   % ∂Q/∂x1, ∂Q/∂x3
    Hk(3,1) = x5;  Hk(3,5) = x1;   % ∂P/∂x1, ∂P/∂x5
    ...
    % --- Post-update clamping ---
    % xhat_plus(1:5,k)  = min(max(xhat_plus(1:5,k), -1e6), 1e6);
...
save('EKF_variance.mat', 'ekf_var');

%% Compute F now

%% === EKF Parameter Estimate Plot ===
%% === EKF Parameter Estimate Plot (Using x_true) ===
true_params = x_true(6:10, :);  % Fetch true parameter trajectories from x_true
...
% %% === EKF Dynamic State Estimate Plot ===
% state_names = {'Vt (x1)', 'iqpfc (x2)', 'Iq (x3)', 'Pref (x4)', 'Id (x5)'};
% 
% figure;
% for i = 1:5
%     subplot(3, 2, i);
...
%     grid on;
% end
% sgtitle('EKF Dynamic State Estimates (x1 to x5)');
